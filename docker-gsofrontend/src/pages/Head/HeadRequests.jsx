...
const MENU_ITEMS = [
  ...
  { text: "Schedules", to: "/adminschedules", icon: "M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z M16 2v4 M3 10h18 M8 2v4 M17 14h-6 M13 18H7 M7 14h.01 M7 16h.01" },
          ...
          {requests.length > 0 ? (
            requests.map((request) => (
              ...
              >
                ...
                <td className="p-3">
                  {maintenanceTypeMap[request.maintenance_type_id] || "Unknown"}
                </td>
                <td className="p-3">
                  <span
...
const HeadRequests = () => {
  ...
  const handleRowClick = useCallback(
    (id) => {
      const path =
        selectedTab === "Approved" || selectedTab === "Disapproved"
          ? `/viewmaintenancerequestform/${id}`
          : `/headmaintenancerequestform/${id}`;
      navigate(path);
    },
    [navigate, selectedTab]
  );

  const filtered = requests.filter((r) => r.status === selectedTab && r.verified_by !== null);
  const showActions = ["Pending", "Approved", "Disapproved"].includes(selectedTab);
